export function getSystemPrompt(dataSource, dataBase) {
  ...
  \`\`\`
  #23 Substitute the api-key with \`process.env.MONGO_DB_API_KEY\`
  #24 The dataSource is ${dataSource}
  #25 database is ${dataBase}
  #26 The data schema is a collection of json objects, each with this format:
  \`\`\`
  [collection name]: {
    [field name]: {
      type: [data type],
